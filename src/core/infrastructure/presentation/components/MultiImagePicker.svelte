<script lang="ts">
    ...
    $: totalCount = values.length + localImages.length;
    ...
    <div class="picker-head">
        ...
        <button class="picker-btn" type="button" on:click={() => (open = !open)} disabled={isDisabled}>
            <div class="picker-btn-main">
                {#if values[0] || localImages[0]?.preview}
                    <img class="thumb" src={values[0] || localImages[0].preview} alt="" aria-hidden="true" />
                {:else}
