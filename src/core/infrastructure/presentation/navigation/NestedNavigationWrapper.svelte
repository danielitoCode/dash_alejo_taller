<script lang="ts">
    ...
    onMount(() => {
        ...
        if (ENV.databaseId) {
            const channel = `databases.${ENV.databaseId}.collections.product.documents`;
            try {
                stopAppwriteProductRt = client.subscribe(channel, (response) => {
                    try {
                ...
                logger.info(`[stock-rt][appwrite] subscribed ${channel}`);
            } catch (e: any) {
