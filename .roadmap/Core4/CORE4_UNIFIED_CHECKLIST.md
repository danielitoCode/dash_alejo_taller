...
- [ ] **BOTH** Core 2 confirmado: `sale_finance_event` al VERIFIED; UNVERIFIED/DELETED sin finance
- [ ] **BOTH** Core 3: dependencia de `last_unit_cost` documentada (merge Core3 a master recomendado antes del merge Core4)
- [ ] **DASH** Política [POLICY_SALE_FINANCE_CORE4.md](./POLICY_SALE_FINANCE_CORE4.md) aceptada
- [ ] **DASH** [SCHEMA_AUDIT_CORE4.md](./SCHEMA_AUDIT_CORE4.md) completado (actual vs propuesto)
- [ ] **DASH** Decisión de diseño: **Opción A** (`lines_json` en el documento) vs Opción B (colección líneas)
- [ ] **DASH** Lista de atributos Appwrite a provisionar (si aplica `lines_json` u otros)
- [ ] **AT** Política espejada en `.roadmap/Core4/`

**Salida B0:** política + audit + decisión de schema firmes; sin código de feature aún.
...
- [ ] **DASH** Extender entidad `SaleFinanceEvent` con líneas (`SaleFinanceLine`: productId, qty, unitPrice, unitCostSnapshot, lineRevenue, lineCogs, lineMargin)
- [ ] **DASH** Actualizar `buildFinanceEventFromSale` para rellenar snapshot por línea y totales consistentes
- [ ] **DASH** Mapper DTO ↔ dominio (incl. serialización `lines_json` o campos acordados)
- [ ] **DASH** `RegisterSaleFinanceFromVerifiedCaseUse` usa el nuevo contrato sin romper idempotencia por `sale_id`
- [ ] **DASH** Tipado / validaciones: costs ≥ 0, qty > 0, margin = revenue − cogs a nivel doc y línea
- [ ] **AT** Espejo de tipos/`SaleFinanceWrite` alineado al mismo contrato (campos que el operador escribirá en B3)

**Salida B1:** dominio y mappers listos; aún puede faltar wiring completo de confirm si se parte en B2.
...
- [ ] **DASH** Provisionar atributo(s) Appwrite si B0 lo exige; smoke create/read del nuevo campo
...
- [ ] **DASH** Unit: `buildFinanceEventFromSale` con varias líneas y costos distintos
- [ ] **DASH** Unit: margen doc = Σ márgenes línea (o documentar redondeo)
- [ ] **DASH** Unit: mapper round-trip con `lines_json`
