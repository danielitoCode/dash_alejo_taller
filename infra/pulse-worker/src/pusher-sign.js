/** MD5 hex (Pusher body_md5). Pure JS for Worker + Node tests. */
...
/**
 * Build signed Pusher REST trigger request.
 * @returns {{ url: string, body: string }}
 */
export async function buildPusherTriggerRequest({
  appId,
  key,
  secret,
  cluster,
  ...
  const url = `https://api-${cluster || "mt1"}.pusher.com${path}?${query}&auth_signature=${signature}`;
  return { url, body, bodyMd5, path, query };
}

export function authorizePulse(request, apiKey) {
  if (!apiKey) return false;
  const auth = request.headers.get("Authorization") || "";
  const xKey = request.headers.get("X-Api-Key") || "";
  if (xKey && xKey === apiKey) return true;
...
export function corsHeaders(origin, allowedOriginsCsv) {
